...
# By default, read10xCounts assumes input is separate files each corresponding to a 10X sample. Change this with type argument.

sce <- read10xCounts("~/Documents/Rubin Mouse Aging/data/parabiosis/raw_files/10X/", col.names = FALSE, type = "sparse")

# type = "sparse" is a conventional column-sparse compressed matrix format produced by the CellRanger pipeline.
...
cols <- c("animal", "batch", "parabiosis_category", "cell_type", "subpopulation")
...
pc50 <- pca_coord@cell.embeddings # We want cell.embedding which stores the coordinates for each cell in low-dimensional space.
...
# reorder the list
reducedDims(sce) <- reducedDims(sce)[c("PCA", "UMAP", "TSNE")] # Can do this with assays as well.


# Change PCA, UMAP, TSNE column names
colnames(reducedDim(sce, "UMAP")) <- c("UMAP1", "UMAP2")
colnames(reducedDim(sce, "TSNE")) <- c("TSNE1", "TSNE2")
colnames(reducedDim(sce, "PCA")) <-  sub("_", "", colnames(reducedDim(sce, "PCA")))

# Remove current metadata and add cell colors matching Ximerakis & Holton et al. 2023
...
Ximerakis & Holton et al. (2023) Heterochronic parabiosis reprograms the mouse brain transcriptome by shifting aging signatures in multiple cell types. *Nat Aging* 3, 327--345. <DOI:https://doi.org/10.1038/s43587-023-00373-6>.
