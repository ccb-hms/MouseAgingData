...
For this dataset, the authors have already provided us with their exact UMAP and TSNE coordinates, as well as their color scheme representing the cell types from their paper. This can be accessed in the metadata slot of the `SingleCellExperiment` object with the `colData` function.
...
# Add our UMAP to the reducedDims slot
umap2 <- runUMAP(sce, dimred = 'PCA')
reducedDims(sce, "osca_UMAP") <- runUMAP(sce, dimred = 'PCA')
...
reducedDim(sce, "TSNE2") <- TSNE2 # Add another component to reducedDims
# reorder the list
...
Heterochronic parabiosis reprograms the mouse brain transcriptome by shifting aging signatures in multiple cell types. \emph{Nat Aging} 3, 327–345. DOI:https://doi.org/10.1038/s43587-023-00373-6.
